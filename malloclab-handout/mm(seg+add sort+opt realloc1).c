...
void *mm_realloc(void *ptr, size_t size){
    ...
    //需要先考虑前后有无直接利用的空闲块，如果没有则需要调用mm_malloc
        ...
        if(bp == NULL){//需要调用malloc
        ...
        else if(bp == ptr){ //利用了ptr后面的空闲块
            // printf("mm_realloc use recoalesce, bp == ptr\n");
            // printf("bp=%p\tptr=%p\n", bp, ptr);
            return ptr;
        }
        else{ //前面的空闲块被利用
            // printf("mm_realloc use recoalesce, bp != ptr\n");
            // printf("bp=%p\tptr=%p\n", bp, ptr);
            // printf("size=%d\toldsize=%d\tasize=%d\tcopysize=%d\n", 
            //     size,old_size, asize, copysize);
            memmove(bp, ptr, copysize);//如果会导致原数据被覆盖，memcpy就是不安全的复制，需要用memmove
            return bp;
        }
    }
}
//合并空闲块，使得达到大小asize的要求
static void *recoalesce(void *bp, size_t asize){
    ...
    if(prev_alloc && next_alloc)//前后均不为空闲块
        ...
        if(prev_size + next_size + bp_size < asize) 
            return NULL;
        //优化版本2：对于前后都可以合并的情况，优先合并小的空闲块
        else if(next_size + bp_size >= asize && prev_size + bp_size >= asize){
            if(prev_size < next_size)//优先使用小的空闲块
                res = prev_merge(prev, bp, asize);
            else
                res = next_merge(next, bp, asize);
            // printf("recoalesce return: res = %p\n", res);
        }
        else if(prev_size + bp_size >= asize){
            res = prev_merge(prev, bp, asize);
            // printf("recoalesce return: res = %p\n", res);
        }
        else if(next_size + bp_size >= asize){
            res = next_merge(next, bp, asize);
        }
        else{//需要合并三个块
            void *temp = next_merge(next, bp, next_size + bp_size);
            res = prev_merge(prev, temp, asize);
            // printf("recoalesce return: res = %p\n", res);
        }
        return res;
    }
...
//确定前为空闲块的时候，进行合并，以获得合适的大小asize的可利用块
static void *prev_merge(void *prev, void *bp , size_t asize){
    // printf("--------prev_merge--------\n");
    size_t prev_size = GET_SIZE(HDRP(prev));
    size_t bp_size = GET_SIZE(HDRP(bp));
    if(prev_size + bp_size < asize) return NULL;
        ...
        PUT(HDRP(NEXT_BLKP(prev)), PACK(asize, 1));
        PUT(FTRP(NEXT_BLKP(prev)), PACK(asize, 1));
        void *res = NEXT_BLKP(prev);
        coalesce(prev);
        return res;
    }
    else{//全部使用
...
//确定后面为空闲块的时候，进行合并，以获得合适的大小asize的可利用块
static void *next_merge(void *next, void *bp, size_t asize){
    // printf("--------next_merge--------\n");
    size_t next_size = GET_SIZE(HDRP(next));
    size_t bp_size = GET_SIZE(HDRP(bp));
...
/*segregated free list + address sorted + optimized mm_realloc version2
Results for mm malloc:
trace  valid  util     ops      secs  Kops
 0       yes   99%    5694  0.000786  7244
 1       yes  100%    5848  0.000773  7569
 2       yes   99%    6648  0.000929  7155
 3       yes  100%    5380  0.000771  6976
 4       yes   99%   14400  0.001275 11293
 5       yes   94%    4800  0.001335  3596
 6       yes   93%    4800  0.001438  3339
 7       yes   55%   12000  0.016544   725
 8       yes   51%   24000  0.053823   446
 9       yes   94%   14401  0.048954   294
10       yes   57%   14401  0.002462  5850
Total          85%  112372  0.129089   870
